...
export abstract class BaseCrudController<T = any> extends BaseController {
  ...
  /** 将 API 输入的 camelCase 数据转为数据库需要的 snake_case */
  ...
  private registerCreate() {
    ...
    this.addRoute({
      ...
      handler: async (req: Request, res: Response) => {
        const item = await this.repository.create(this.transformInput(req.body));
    ...
    this.addRoute({
      ...
      handler: async (req: Request, res: Response) => {
        const item = await this.repository.update(
          req.params.id,
          req.query.tenantId as string,
          this.transformInput(req.body) as Partial<T>,
        );
